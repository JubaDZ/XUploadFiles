...
function GenerateCaptcha(_background_ ) {
	...
	_captcha_ = 'ajax/index.php?captcha&background='+_background_+'&' + Math.random();
	if ( $( "#captcha_1" ).length )
...
function GetTotalPages() {
    $.get("ajax/index.php?totalpages", function(data, status) {
...
function ExtReadMore(num) {
	...
	$('#extensions_mobile').html('<code><i class="icon-spin4 animate-spin"></i> ' + Loading + ' ...</code>'); 
	    $.get("ajax/index.php?getextensions" , function(data, status) {
        if (status == 'success'){
...
function LoadStatsTables(num,id,DivId) {
    ...
    $.get("ajax/index.php?stats&currentpage=" + num + "&id="+id, function(data, status) {
        if (status == 'success') {
            if (data['status'])  
			{
...
if(IsGetProfile)
  ...
  function GetProfile() {
	  ...
	  $("#progressFree").html( Loading + ' ...'); 
	     $.get("ajax/index.php?getspace", function(data, status) {
            if (status == 'success') {
