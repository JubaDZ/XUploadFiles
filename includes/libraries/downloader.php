...
class downloader {
	...
	function header($size,$seek_start=null,$seek_end=null) {
		...
		else { header('Content-type: File Transfer'); }
	...
	function download() {
		...
		if ($this->data_type == 0) //download from a file
		{
			...
			$size = ($this->set_filesize>0) ? preg_replace("/[^0-9]+/", "",$this->set_filesize) : filesize($this->data) ;
			isset($_GET[$this->set_szParameter]) ? $size = preg_replace("/[^0-9]+/", "",$_GET[$this->set_szParameter]) : $size ; 
